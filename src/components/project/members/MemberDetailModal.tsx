        ...
        <div className="px-4 py-3 bg-component-secondary-background rounded-lg">
          {
            member.currentTask.length > 0 ? (
              member.currentTask.map((task, idx) => (
                <p
                  key={idx}
                  className="text-text-secondary py-1 hover:text-blue-400 cursor-pointer transition-colors"
            ...
            {member.workingHours ? (
              <>
                ...
                {member.workingHours.timezone && (
                  <span className="ml-2 text-text-secondary text-sm">{member.workingHours.timezone === "Asia/Seoul" ? "한국 표준시 (KST)" :
          ...
          {member.socialLinks && member.socialLinks.length > 0 ? (
            member.socialLinks.map((link, index) => (
              <a
                ...
                className="px-3 py-2 bg-component-secondary-background hover:bg-component-tertiary-background rounded-lg text-text-secondary transition-colors flex items-center"
              >
