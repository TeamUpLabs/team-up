  ...
  return (
    <div className={isPreview ? 'rounded-lg overflow-hidden bg-component-background p-3 origin-top-left' : 'col-span-1 sm:col-span-2 bg-component-background p-4 sm:p-6 rounded-lg shadow-md border border-component-border'}>
      ...
      <div className={`flex mb-4 bg-component-secondary-background rounded-lg overflow-hidden w-fit p-1 ${isPreview ? 'mb-1' : ''}`}>
        <div className={`${isPreview ? 'px-2 py-1' : 'px-6 py-2'} rounded-lg ${isPreview ? 'text-xs' : 'text-base'} font-semibold text-text-secondary bg-component-tertiary-background_hover w-fit`}>회의 <span className="ml-1 text-xs font-bold text-gray-400 animate-pulse">N</span></div>
        <div className={`${isPreview ? 'px-2 py-1' : 'px-6 py-2'} rounded-lg ${isPreview ? 'text-xs' : 'text-base'} font-semibold text-text-secondary bg-component-tertiary-background_hover w-fit`}>작업 <span className="ml-1 text-xs font-bold text-gray-400 animate-pulse">N</span></div>
      </div>
          ...
          <div key={index} className={`bg-component-secondary-background border border-component-border rounded-xl flex flex-col gap-3 ${isPreview ? 'p-2' : 'p-4'}`}>
            ...
            <div className={`flex items-center justify-between mt-2 ${isPreview ? 'mb-1' : ''}`}>
              ...
              <div className="flex ml-auto gap-1">
                <div className={`${isPreview ? 'w-6 h-6' : 'w-7 h-7'} bg-gray-300 rounded-full border-2 border-component-border animate-pulse`}></div>
                <div className={`${isPreview ? 'w-6 h-6' : 'w-7 h-7'} bg-gray-300 rounded-full border-2 border-component-border -ml-2 animate-pulse`}></div>
              </div>
            </div>
          </div>
        ))}
